...
`scripts/show-pairing-code.sh` only when pairing a phone.

The public `/health` endpoint never includes credentials. `/configure` exchanges the one-time code
for the independent gateway token over private HTTPS. Chat and message endpoints are read-only in
this milestone.
