...
f.onsubmit=async(e)=>{e.preventDefault();updateTheme();s.className='hint';s.textContent='Testing…';try{let token=initial.gatewayToken||'';if(token){const test=await fetch('/v1/chats?limit=1',{headers:{authorization:'Bearer '+token}});if(!test.ok)throw new Error('Saved connection failed with '+test.status);}else{const r=await fetch('/pair',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code:document.getElementById('code').value.trim()})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Pairing failed');token=j.gatewayToken;}const chosen=selected(),custom=themes.filter(t=>!t.builtIn).slice(0,20),quickReplies=[];for(let i=0;i<8;i++){const reply=document.getElementById('quickReply'+i).value.trim();if(reply)quickReplies.push(reply);}const value={gatewayURL:location.origin,gatewayToken:token,theme:chosen,themes:custom,quickReplies:quickReplies,textSize:chosen.size>=26?'large':'normal',refresh:Number(document.getElementById('refresh').value)};location.href='pebblejs://close#'+encodeURIComponent(JSON.stringify(value));}catch(err){s.className='error';s.textContent=err.message;}};
</script></body></html>`;
}
