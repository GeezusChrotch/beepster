...
<script>const f=document.getElementById('form'),s=document.getElementById('status');let initial={};
try{initial=JSON.parse(decodeURIComponent(location.hash.slice(1)||'%7B%7D'));}catch(e){}
const paired=Boolean(initial.gatewayToken);if(paired){document.getElementById('pairing').hidden=true;document.getElementById('intro').textContent='Adjust Beepster without pairing again.';document.getElementById('save').textContent='Test connection, save & apply';s.textContent='Your gateway credential remains on the phone.';}
if(initial.theme)document.getElementById('theme').value=initial.theme;if(initial.textSize)document.getElementById('textSize').value=initial.textSize;if(typeof initial.refresh==='number')document.getElementById('refresh').value=String(initial.refresh);
f.onsubmit=async(e)=>{e.preventDefault();s.className='hint';s.textContent='Testing…';try{let token=initial.gatewayToken||'';if(token){const test=await fetch('/v1/chats?limit=1',{headers:{authorization:'Bearer '+token}});if(!test.ok)throw new Error('Saved connection failed with '+test.status);}else{const r=await fetch('/pair',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code:document.getElementById('code').value.trim()})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Pairing failed');token=j.gatewayToken;}const value={gatewayURL:location.origin,gatewayToken:token,theme:document.getElementById('theme').value,textSize:document.getElementById('textSize').value,refresh:Number(document.getElementById('refresh').value)};location.href='pebblejs://close#'+encodeURIComponent(JSON.stringify(value));}catch(err){s.className='error';s.textContent=err.message;}};</script></body></html>`;
}
